...
namespace ListagemDeCervejaInterface
{
    ...
    class Program
    {
        ...
        {
            AdicionaCerveja();
            cervejaController.GetListaCervejas().ForEach(x => Console.WriteLine("Id: {0,2} Nome: {1,-15} Litros: {2,4} Alcool: {3,4} Valor: {4,8}"
                , x.Id, x.Nome, x.Litros, x.Alcool, x.Valor.ToString("C2")));
            Console.WriteLine("\nO Total de cerveja é: {0} litros.",cervejaController.GetRetornaLitrosTotal());
            Console.WriteLine("O valor total em cerveja é de: {0}", cervejaController.GetRetornaValorTotal().ToString("C2"));
            Console.WriteLine("\n\nPara calcular o indice de alcool no seu sangue digite seu peso:");
            int.TryParse(Console.ReadLine(), out int peso);
            var teorAlcoolSangue = cervejaController.GetRetornaTeorAlcoolSanguel(peso);
            Console.WriteLine("\nO teor alcólico no sangue é de {0} g de álcool/litro de sangue.", teorAlcoolSangue.ToString("N2") );
        ...
        public static void  AdicionaCerveja ()
        {
            ...
            cervejaController.GetAdicionaCerveja(new Cerveja()
            { 
              Nome = nomeCerveja,
